...
from app.service.booking.product import create_new_product, delete_product_by_id, update_product_by_id
...
@router.put("/{product_id}", response_model=ProductResponse, dependencies=[BusinessAndEmployeesSession])
async def update_product(db: DBSession, product_id: int, product_update: ProductUpdate, request: Request):
    return await update_product_by_id(db, product_id, product_update, request)
